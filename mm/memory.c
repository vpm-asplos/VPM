		     ...
		     unsigned long addr_hint, unsigned int pages_per_huge_page)
{
	...
	if(current != NULL && current->mm != NULL && current->mm->pstore != NULL) {
	  // convert struct page to physical page...
	  unsigned long pfn = page_to_pfn(page);
	  unsigned long index = 0;
	  for(index = 0; index < current->mm->pstore->cnt; index ++) {
	    if(pfn == current->mm->pstore->pages[index].paddr) {
	       printk("Skipped clearing huge page! pfn: %p", pfn);
	      return;
	    }
	  }
	  // printk("Cannot find the physical page! pfn: %lx, pid: %u, addr_hint: %p, addr: %p", pfn, current->pid, addr_hint, addr);
	}
